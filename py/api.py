...
async def getModelsList(request):
    if "type" in request.rel_url.query:
        ...
        return web.json_response(manager.get_model_lists(context, type))
    else:
        return web.Response(status=400)
...
async def getModelsThumbnail(request):
    ...
    if "limit" in request.rel_url.query:
        ...
        limit = int(limit)
    context = execution_context.ExecutionContext(request)
    checkpoints = folder_paths.get_filename_list(context, "checkpoints_thumb")
    loras = folder_paths.get_filename_list(context, "loras_thumb")
    checkpoints_full = []
    loras_full = []
    context = execution_context.ExecutionContext(request)
    ...
    for index, i in enumerate(checkpoints):
        full_path = folder_paths.get_full_path(context, 'checkpoints_thumb', str(i))
        if full_path:
            checkpoints_full.append(full_path)
    for index, i in enumerate(loras):
        full_path = folder_paths.get_full_path(context, 'loras_thumb', str(i))
        if full_path:
